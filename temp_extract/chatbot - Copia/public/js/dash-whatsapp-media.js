...
(function () {
    ...
    var MEDIA_TYPES = ['image', 'video', 'audio', 'document'];
    ...
    function inferTypeFromFile(file) {
        ...
        if (mime.indexOf('image/') === 0 || ext === 'jpg' || ext === 'jpeg' || ext === 'png' || ext === 'gif' || ext === 'webp') {
            return 'image';
        }
    ...
    function renderMessageInner(msg) {
        ...
        var html = '';
        if (type === 'image' || (!type && /\.(jpe?g|png|gif|webp)(\?|$)/i.test(url))) {
            html =
                '<a href="' +
                esc(url) +
                ...
                '<img src="' +
                esc(url) +
                '" alt="Imagem" class="chat-media-img" loading="lazy">' +
                '</a>';
        } else if (type === 'video') {
